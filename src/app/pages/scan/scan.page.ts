...
export class ScanPage implements AfterViewInit {
  ...
  async ngAfterViewInit() {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if (!subject || !subject.answerKey) {
        void this.presentAlert('No answer key found for this subject!');
        return;
      }
      this.answerKey = {};
  ...
  private initOpenCVMatsAndCapture(videoEl: HTMLVideoElement) {
    if (typeof (window as any).cv !== 'undefined') {
      ...
      void this.presentAlert('OpenCV is not available. Please try again later.');
    }
  }
  ...
  async onStartCameraButtonClick() {
    ...
    if (!ok) {
      await this.presentAlert('Camera permission is required to scan. Please allow camera access in app settings.');
  ...
  startCameraView() {
    ...
    this.cameraService.attachToVideo(videoEl).then(() => {
      ...
      if (msg.toLowerCase().includes('permission')) {
        void this.presentAlert('Camera permission denied. Please allow camera access in app settings.');
      } else {
        void this.presentAlert('Error accessing camera: ' + msg);
      }
      this.showCamera = false;
    });
  }
  ...
  processVideo() {
    try {
      ...
      if (!ctx) {
        void this.presentAlert('Could not get canvas context');
        return;
      }
  ...
  processSheet(ctx: CanvasRenderingContext2D) {
    if (!this.latestWarpedMat || this.latestWarpedMat.empty()) {
      void this.presentAlert('processSheet: No warpedMat available.');
      return;
    }
